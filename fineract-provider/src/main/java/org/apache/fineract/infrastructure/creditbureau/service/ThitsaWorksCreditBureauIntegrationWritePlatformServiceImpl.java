...
public class ThitsaWorksCreditBureauIntegrationWritePlatformServiceImpl implements ThitsaWorksCreditBureauIntegrationWritePlatformService {
    ...
    private final DataValidatorBuilder baseDataValidator = new DataValidatorBuilder(dataValidationErrors)
            .resource("creditbureau.configuration");
    ...
    public CreditBureauReportData getCreditReportFromThitsaWorks(final JsonCommand command) {
        String searchResult = "235";
        Long uniqueID = this.extractUniqueId(searchResult);
        CreditBureauReportData creditBureauReportData = null;
        return creditBureauReportData;
    }

    /*
     * this.context.authenticatedUser(); String nrcId = command.stringValueOfParameterNamed("NRC"); String bureauID =
     * command.stringValueOfParameterNamed("creditBureauID"); Integer creditBureauId = Integer.parseInt(bureauID);
     ...
     * CreditBureauConfigurations.subscriptionId.toString()); String subscriptionKey =
     * getCreditBureauConfiguration(creditBureauId, CreditBureauConfigurations.subscriptionKey.toString());
     *
     * CreditBureauToken creditbureautoken = createToken(creditBureauId.longValue()); token =
     * creditbureautoken.getCurrentToken();
     *
     * LOG.info("token processed done {}", token); // will use only "NRC" part of code from common http method to get
     * data based on nrc String process = "NRC"; CreditBureauConfiguration searchURL =
     * this.configDataRepository.getCreditBureauConfigData(creditBureauId, "searchurl"); String url =
     * searchURL.getValue(); String nrcUrl = url + nrcId; LOG.info("searchURL: {} ", nrcUrl);
     *
     * String searchResult = this.okHttpConnectionMethod(userName, password, subscriptionKey, subscriptionId, nrcUrl,
     * token, null, 0L, nrcId, process);
     *
     * if (process.equals("NRC")) { Long uniqueID = this.extractUniqueId(searchResult); LOG.info("uniqueID : {} ",
     * uniqueID); process = "CreditReport"; CreditBureauConfiguration creditReportURL =
     * this.configDataRepository.getCreditBureauConfigData(creditBureauId, "creditReporturl"); url =
     * creditReportURL.getValue(); String creditReportUrl = url + uniqueID; LOG.info("creditReporturl: {} ",
     * creditReportUrl);
     *
     * searchResult = this.okHttpConnectionMethod(userName, password, subscriptionKey, subscriptionId, creditReportUrl,
     * token, null, uniqueID, null, process);
     *
     * }
     *
     ...
     */
    @Transactional
    public CreditBureauReportData getCreditReportFromThitsaWorksTesting(final String searchResult) {
    ...
    public String getCreditBureauConfiguration(Integer creditBureauId, String configurationParameterName) {
        ...
        try {
            ...
            if (creditBureauConfigurationValue.isEmpty()) {
                ...
                throw new PlatformApiDataValidationException("validation.msg.creditBureau.configuration.is.not.available",
                        "validation.msg.creditBureau.configuration.is.not.available", dataValidationErrors);
            }
    ...
    private void handleAPIIntegrityIssues(String httpResponse) {
        ...
        throw new PlatformApiDataValidationException(httpResponse, httpResponse, dataValidationErrors);
    ...
    private void handleMultipleNRC(String nrc) {
        ...
        throw new PlatformApiDataValidationException(showMessageForMultipleNRC, showMessageForMultipleNRC, dataValidationErrors);
